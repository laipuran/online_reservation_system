...
创建预约。服务端根据服务时长计算 `end_time`，初始状态为 `pending`，并向对应服务提供者发送站内通知。同一服务下 `pending` 或 `confirmed` 预约的时间段不能重叠。
...
| start_time | string | 是 | 预约开始时间，ISO 8601 格式；不能早于当前日期，不能晚于当前时间 3 个月后 |
| note | string | 否 | 用户备注 |
